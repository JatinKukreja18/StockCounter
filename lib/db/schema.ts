...
export const sessionStatus = pgEnum("session_status", ["draft", "open", "closed"]);
export const issueStatus = pgEnum("issue_status", ["open", "accepted", "corrected", "voided"]);
...
export const products = pgTable("products", {
  id: uuid("id").primaryKey().defaultRandom(),
  stockImportId: uuid("stock_import_id").notNull().references(() => stockImports.id, { onDelete: "cascade" }),
  ...
  uniqueIndex("products_import_store_sku_unique").on(table.stockImportId, table.store, table.sku)
]);
...
export const stockBatches = pgTable("stock_batches", {
  ...
  uniqueIndex("stock_batches_product_key_unique").on(table.productId, table.batchKey),
...
export const sessions = pgTable("sessions", {
  ...
  store: text("store").notNull(),
  stockImportId: uuid("stock_import_id").notNull().references(() => stockImports.id),
...
export const sessionProducts = pgTable("session_products", {
  ...
  stockBatchId: uuid("stock_batch_id").notNull().references(() => stockBatches.id),
  systemQtySnapshot: numeric("system_qty_snapshot", { precision: 14, scale: 3 }).notNull(),
  stockVersionSnapshot: timestamp("stock_version_snapshot", { withTimezone: true }).notNull()
}, (table) => [
...
export const sessionAssignments = pgTable("session_assignments", {
  ...
  index("session_assignments_session_idx").on(table.sessionId)
]);
...
export const countEntries = pgTable("count_entries", {
  ...
  createdOnDeviceAt: timestamp("created_on_device_at", { withTimezone: true }).notNull(),
  syncedAt: timestamp("synced_at", { withTimezone: true }).notNull().defaultNow(),
  correctedFromId: uuid("corrected_from_id")
}, (table) => [
  uniqueIndex("count_entries_local_entry_unique").on(table.localEntryId),
  index("count_entries_session_product_idx").on(table.sessionId, table.productId)
]);
...
export const syncIssues = pgTable("sync_issues", {
  ...
  index("sync_issues_status_idx").on(table.status)
]);
