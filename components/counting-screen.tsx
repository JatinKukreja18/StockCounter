...
import type { CountEntry, CountSession, LocalCountEntry, Product, StockBatch } from "@/lib/types";
...
export function CountingScreen() {
  ...
  const sessionProducts = useMemo(
    () => products.filter((product) => activeSession?.productIds.includes(product.id)),
    [activeSession, products]
  );
  const filteredProducts = useMemo(() => {
    const value = query.trim().toLowerCase();
    if (!value) return sessionProducts;
    return sessionProducts.filter((product) =>
      product.barcode.toLowerCase().includes(value) ||
      product.sku.toLowerCase().includes(value) ||
      product.name.toLowerCase().includes(value)
    );
  }, [query, sessionProducts]);
  const incompleteProducts = filteredProducts.filter((product) => !countedProductIds.has(product.id));
  const completedProducts = filteredProducts.filter((product) => countedProductIds.has(product.id));
  const totalIncompleteProducts = sessionProducts.filter((product) => !countedProductIds.has(product.id)).length;
  const activeSessionEntries = entries.filter((entry) => entry.sessionId === activeSessionIdForCounts);
  const selectedServerCount = selected ? serverCountsByProduct.get(selected.id) ?? 0 : 0;
  const selectedLocalCount = selected ? localCountsByProduct.get(selected.id) ?? 0 : 0;
  ...
  function submitBarcode(rawBarcode: string, fromHardwareScanner: boolean) {
    ...
    const activeMatch = sessionProducts.find((product) => product.barcode === barcode);
    if (activeMatch) {
      chooseProduct(activeMatch, fromHardwareScanner);
      return;
    }

    const matchingProduct = products.find((product) => product.barcode === barcode);
    if (matchingProduct) {
      const otherSessions = sessions.filter((session) =>
        session.id !== activeSession.id && session.productIds.includes(matchingProduct.id)
      );
      const destination = otherSessions.map((session) => session.name).join(", ");
      setNotice({
        tone: "red",
        text: destination
    ...
    setNotice({ tone: "red", text: `Barcode ${barcode} was not found in the cached product list.` });
  }
  ...
  async function correctUnsyncedProductTotal(targetTotal: number) {
    ...
    if (!Number.isFinite(targetTotal) || targetTotal < 0) {
      setNotice({ tone: "red", text: "Enter a valid non-negative product total." });
      return;
    }
    ...
    const editableTotal = selectedEditableEntries.reduce((sum, entry) => sum + entry.quantity, 0);
    const targetEditableTotal = targetTotal - lockedTotal;
    const difference = targetEditableTotal - editableTotal;
    if (difference === 0) {
      showTemporaryNotice({ tone: "green", text: `${selected.name} is already at ${formatNumber(targetTotal)}.` });
      return;
    }
  ...
  function renderProductRow(product: Product, isCounted: boolean) {
    const productTotal = (serverCountsByProduct.get(product.id) ?? 0) + (localCountsByProduct.get(product.id) ?? 0);
    const hasLocalCount = (localCountsByProduct.get(product.id) ?? 0) > 0;
    return (
      <button key={product.id} onClick={() => chooseProduct(product)} className="w-full text-left">
        <Card className="flex items-center gap-3 rounded-xl p-3.5 shadow-none transition hover:border-[#63a982] active:bg-[#f7f9f7]">
          <span className={`grid size-10 shrink-0 place-items-center rounded-xl ${isCounted ? "bg-[#e9f6ef] text-[#18794e]" : "bg-[#eef1ef] text-[#68726c]"}`}>
            {isCounted ? <Check size={19} /> : <PackageCheck size={19} />}
          </span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-bold">{product.name}</span>
            <span className="mt-1 block text-xs text-[#7a847e]">{product.sku} · {product.barcode}</span>
          </span>
          <span className="flex shrink-0 items-center gap-2">
            {isCounted ? (
              <span className="text-right">
                <span className="block text-sm font-black">{formatNumber(productTotal)}</span>
                <span className={`block text-[10px] font-bold ${hasLocalCount ? "text-[#b45309]" : "text-[#18794e]"}`}>
    ...
    <div className="mx-auto min-h-[calc(100dvh-4rem)] max-w-xl px-4 pb-32 pt-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          ) : <h1 className="truncate text-xl font-black tracking-tight">{activeSession?.name ?? "No assigned session"}</h1>}
        </div>
      ...
      {activeSession && (
        <div className="mb-4 flex items-center justify-between rounded-xl bg-[#edf5f0] px-4 py-3">
          <div>
            ...
            <p className="text-sm font-black text-[#18794e]">{sessionProducts.length - totalIncompleteProducts} of {sessionProducts.length} products counted</p>
          </div>
          <span className="text-2xl font-black text-[#18794e]">
            {sessionProducts.length ? Math.round(((sessionProducts.length - totalIncompleteProducts) / sessionProducts.length) * 100) : 0}%
          </span>
        </div>
      )}
      ...
      {notice && (
        <div className={`animate-enter mb-4 flex items-start gap-3 rounded-xl border p-3 text-sm font-semibold ${notice.tone === "green" ? "border-[#bfe4cf] bg-[#e9f6ef] text-[#12673f]" : "border-[#f2c4bf] bg-[#fff0ee] text-[#9e251b]"}`}>
          ...
          {notice.undoLocalEntryId && (
            <button onClick={() => void undoSavedCount(notice.undoLocalEntryId!)} className="shrink-0 font-black underline underline-offset-2">Undo</button>
          )}
          <button onClick={() => setNotice(null)} className="shrink-0" aria-label="Dismiss message"><X size={16} /></button>
        </div>
      )}
      ...
      <div className="relative mb-5">
        ...
        />
        <button onClick={() => setScannerOpen(true)} className="absolute right-2 top-2 grid size-10 place-items-center rounded-xl bg-[#18211d] text-white" aria-label="Scan with camera"><ScanLine size={21} /></button>
      </div>
      ...
      {!activeSession ? (
        <Card className="border-dashed px-6 py-12 text-center shadow-none">
          ...
          <p className="mt-2 text-sm text-[#7a847e]">Ask an administrator to add you to an open session.</p>
        </Card>
      ) : (
        <div className="space-y-5">
          <section>
            <div className="mb-3 flex items-end justify-between">
              <div>
                ...
                <p className="text-xs text-[#7a847e]">Tap a product or scan its barcode.</p>
              </div>
              <Badge>{query ? incompleteProducts.length : totalIncompleteProducts}</Badge>
            </div>
            {incompleteProducts.length > 0 ? (
              <div className="space-y-2">{incompleteProducts.map((product) => renderProductRow(product, false))}</div>
            ) : (
              <Card className="py-9 text-center shadow-none">
                <Check className="mx-auto mb-3 text-[#18794e]" />
                <p className="font-bold">{query ? "No uncounted products match" : "Every product has been counted"}</p>
              </Card>
            )}
          </section>
          ...
          <section>
            ...
            >
              <span>
                <span className="block text-sm font-black">Counted products</span>
                <span className="block text-xs text-[#7a847e]">Reopen a product to add another count</span>
              </span>
              <span className="flex items-center gap-2">
                <Badge tone="green">{completedProducts.length}</Badge>
                <ChevronDown size={18} className={`transition ${countedOpen ? "rotate-180" : ""}`} />
              </span>
            </button>
            {countedOpen && (
              <div className="animate-enter mt-2 space-y-2">
                {completedProducts.length > 0
                  ? completedProducts.map((product) => renderProductRow(product, true))
                  : <p className="py-4 text-center text-sm text-[#7a847e]">No counted products match.</p>}
              </div>
            )}
          </section>
          ...
          <section>
            ...
            >
              <span>
                ...
                <span className="block text-xs text-[#7a847e]">Counts saved on this device for this session</span>
              </span>
              <span className="flex items-center gap-2">
                <Badge>{activeSessionEntries.length}</Badge>
                <ChevronDown size={18} className={`transition ${entriesOpen ? "rotate-180" : ""}`} />
              </span>
            </button>
            {entriesOpen && (
              <div className="animate-enter mt-2 space-y-2">
                {activeSessionEntries.length === 0 ? (
                  <Card className="py-8 text-center shadow-none"><History className="mx-auto mb-3 text-[#9aa29d]" /><p className="font-bold">No entries on this device yet</p></Card>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#18211d]/45 p-0 sm:items-center sm:p-4" onMouseDown={(event) => {
          if (event.target === event.currentTarget) closeProduct();
        }}>
          <div role="dialog" aria-modal="true" aria-labelledby="count-product-title" className="max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
            <div className="sticky top-0 z-10 border-b border-[#e8ece9] bg-[#fbfcfb] p-5">
              <div className="mb-2 flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 id="count-product-title" className="text-xl font-black leading-tight">{selected.name}</h2>
                </div>
                <button onClick={closeProduct} className="grid size-9 shrink-0 place-items-center rounded-full bg-[#eef1ef] text-[#68726c]" aria-label="Close product"><X size={17} /></button>
              </div>
            ...
            <div className="safe-bottom p-5">
              {notice && (
                <div className={`mb-4 flex items-start gap-3 rounded-xl border p-3 text-sm font-semibold ${notice.tone === "green" ? "border-[#bfe4cf] bg-[#e9f6ef] text-[#12673f]" : "border-[#f2c4bf] bg-[#fff0ee] text-[#9e251b]"}`}>
                  {notice.tone === "green" ? <Check className="mt-0.5 shrink-0" size={17} /> : <AlertCircle className="mt-0.5 shrink-0" size={17} />}
                  <span>{notice.text}</span>
                </div>
              )}
              <div className="mb-3 flex items-center justify-between">
                <label className="text-sm font-bold">Additional quantity found now</label>
              ...
              <div className="mb-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2 rounded-xl bg-[#f4f6f4] px-4 py-3 text-center">
                <span>
                  <span className="block text-[10px] font-bold uppercase tracking-wider text-[#7a847e]">Already counted</span>
                  <span className="tabular text-lg font-black">{formatNumber(countedQty)}</span>
                </span>
                <span className="font-black text-[#7a847e]">+</span>
                <span>
                  <span className="block text-[10px] font-bold uppercase tracking-wider text-[#7a847e]">This entry</span>
                  <span className="tabular text-lg font-black">{formatNumber(quantity)}</span>
                </span>
              ...
              <Button size="lg" className="w-full" onClick={saveCount} disabled={!selectedBatch || (countedQty > 0 && quantity === 0)}><PackageCheck size={20} /> Add {formatNumber(quantity)} → total {formatNumber(countedQty + quantity)}</Button>
              {selectedEditableEntries.length > 0 && (
                correctingTotal ? (
                  <div className="mt-3 rounded-xl border border-[#dfe5e1] bg-[#fbfcfb] p-3">
                    <label htmlFor="correct-product-total" className="text-sm font-bold">Correct product total</label>
                    <p className="mt-1 text-xs text-[#7a847e]">Changes unsynced counts on this device. Synced quantities remain locked.</p>
                    <div className="mt-3 flex gap-2">
                      <input
                        id="correct-product-total"
                        value={correctedTotal}
                        onChange={(event) => setCorrectedTotal(Math.max(0, Number(event.target.value) || 0))}
                        type="number"
                        min="0"
                        inputMode="decimal"
                      ...
                      />
                      <Button variant="secondary" onClick={() => setCorrectingTotal(false)}>Cancel</Button>
                      <Button onClick={() => void correctUnsyncedProductTotal(correctedTotal)}>Apply</Button>
                    </div>
                  </div>
                ) : (
              ...
              <p className="mt-2 text-center text-[11px] text-[#7a847e]">Each saved entry is added to the product total and kept safely on this device</p>
            </div>
          </div>
        </div>
      )}
