...
import {
  AuthMethodToggle,
  type AuthMethod
} from "@/components/auth/auth-method-toggle";
    ...
    try {
      ...
      setMessage(
        reason instanceof Error
          ? reason.message
          : `Could not ${editing ? "update" : "create"} account. Check your connection and try again.`
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
      <Card className="p-5">
          ...
          {authMethod === "phone" ? (
            <>
              ...
              <input
                ...
                maxLength={6}
                placeholder={
                  editing ? "New 6-digit PIN (optional)" : "6-digit PIN"
                }
                className="h-11 w-full rounded-xl border border-[#dfe5e1] px-3 text-sm"
              />
            </>
        ...
        {message && (
          <p className="mt-3 rounded-xl bg-[#eef2ef] p-3 text-xs font-semibold">
            {message}
          </p>
        )}
        {error && (
          <p className="mt-3 rounded-xl bg-[#fff0ee] p-3 text-xs font-semibold text-[#9e251b]">
            {error}
          </p>
        )}
      </Card>
            ...
            {users.map((user) => (
              <div key={user.id} className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-[#7a847e]">
