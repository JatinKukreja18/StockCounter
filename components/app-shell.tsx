    ...
    <div className="min-h-dvh">
      <header className="sticky top-0 z-40 border-b border-[#e4e8e5]/90 bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-6">
          <Link
            href={isCount ? "/count" : "/admin"}
            className="flex items-center gap-3"
          >
            <div className="grid size-9 place-items-center rounded-xl bg-[#18794e] text-lg font-black text-white">
              S
            </div>
            <div>
              <div className="text-sm font-black tracking-[-.01em]">
                SEKAI ICHIBA
              </div>
              <div className="text-[10px] font-bold uppercase tracking-[.18em] text-[#7a847e]">
                Stock Count
              </div>
            </div>
          </Link>
      ...
      <div className="mx-auto flex max-w-7xl">
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active
                        ? "bg-[#e9f6ef] text-[#12673f]"
                        : "text-[#68726c] hover:bg-white"
                    )}
                  >
                    <item.icon size={18} />
                    {item.label}
        ...
        <main
          className={cn(
            "min-w-0 flex-1",
            isCount ? "pb-24" : "px-4 py-6 pb-24 lg:px-8 lg:py-8"
          )}
        >
          {children}
        </main>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "text-[#18794e]" : "text-[#7a847e]"
                )}
              >
                <item.icon size={20} />
                {item.label}
